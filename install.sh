...
do_install() {
    ...
    local port https_port admin_pass
    read -r -p "Port to expose the app on [${DEFAULT_PORT}]: " port
    port="${port:-$DEFAULT_PORT}"
    read -r -p "HTTPS port (required for the USB scale) [${DEFAULT_HTTPS_PORT}]: " https_port
...
EOF
    ...
    if command -v ufw >/dev/null 2>&1 && ufw status | grep -q "Status: active"; then
        warn "ufw firewall is active. Allow the app ports with: ufw allow ${port}/tcp && ufw allow ${https_port}/tcp"
    fi
    ...
    echo -e "  Scale URL: ${GREEN}${https_url}${NC}   (packing stations with a USB scale must use this one;"
    echo -e "             accept the self-signed certificate warning once per browser)"
...
do_update() {
    ...
    if [ ! -f "$INSTALL_DIR/nginx/certs/easyship.crt" ]; then
        info "Generating self-signed TLS certificate..."
        bash "$INSTALL_DIR/nginx/gen-certs.sh"
    fi
    if ! grep -q '^APP_HTTPS_PORT=' "$INSTALL_DIR/.env"; then
    ...
    local url="http://$(server_ip)"
    [ "$port" != "80" ] && url="${url}:${port}"
    local https_port; https_port="$(get_env APP_HTTPS_PORT)"
    https_port="${https_port:-$DEFAULT_HTTPS_PORT}"
    local https_url="https://$(server_ip)"
    [ "$https_port" != "443" ] && https_url="${https_url}:${https_port}"
    echo
    ...
    echo -e "  URL:       ${GREEN}${url}${NC}"
    echo -e "  Scale URL: ${GREEN}${https_url}${NC}   (USB scale stations must use this one)"
}
